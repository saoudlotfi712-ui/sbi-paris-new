...
function createDevisNumber(createdAt: string) {
  ...
  return `SBI-DEV-${year}${month}${day}-${hours}${minutes}`;
}
...
export default function DevisPage() {
  ...
  useEffect(() => {
    try {
      const saved =
        localStorage.getItem("sbi-pro-devis") ||
        localStorage.getItem("sbi-pro-invoice");

      if (!saved) {
        return;
      }

      const parsed = JSON.parse(saved) as DevisData;
  ...
  const devisNumber = useMemo(() => {
    ...
    return (
      devis.devisNumber ||
      createDevisNumber(devis.createdAt)
    );
  }, [devis]);
    ...
    return (
      <main style={s.statePage}>
        <div style={s.emptyCard}>
          ...
          <p style={s.emptyText}>
            Préparez d&apos;abord un devis depuis
  ...
  function handlePrintDevis() {
    ...
    if (!printWindow) {
      window.alert(
        "Veuillez autoriser les fenêtres pop-up pour imprimer le devis.",
      );
      return;
    }

    const devisHtml = devisElement.outerHTML;
    ...
    >
      ...
      <div className="devis-screen-wrapper">
        <article
        ...
        >
          <header style={s.header}>
            <div style={s.brandSide}>
              ...
              <div style={s.companyIdentity}>
                <div style={s.brandTitle}>
                  <span style={s.brandBlue}>
                    SBI
                  </span>{" "}
                  <span style={s.brandRed}>
                    PARIS
                  </span>
                </div>
            ...
            <div>
              <div style={s.devisMeta}>
                <div style={s.devisBadge}>
                  <div>DEVIS</div>

                  <div style={s.offerLabel}>
          ...
          </section><section style={s.summaryGrid}>
          ...
          <div style={s.thanks}>
            Une offre préparée pour votre entreprise.
          </div>
          ...
          <section style={s.signatureBox}>
            ...
            <div style={s.clientApprovalColumn}>
              <strong style={s.approvalTitle}>
                BON POUR ACCORD
              </strong>

              <span style={s.approvalText}>
                Signature du client
              </span>
...
const LIGHT = "#f4f7fc";
const BORDER = "#aebbd0";
const TEXT = "#0d2a59";const s: Record<string, CSSProperties> = {
  page: {
    minHeight: "100vh",
    background: "#eef1f5",
